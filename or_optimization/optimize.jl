import Pkg
Pkg.add(["JuMP", "GLPK", "CSV", "DataFrames","Redis"])#julia has to downlaod missing packages
...
@constraint(model, sum((1 -df.is_fraud[i]) * flagged[i] for i in 1:N) <=0.25 * N)
optimize!(model)
...
println("--- Solver Internal Intel ---")
println("Total Transactions (N): ", N)
println("Mean Amount: ", avg_amt)
println("Std Dev: ", std_amt)
println("Current 'Gate' at 1.0: ", avg_amt + 1.0 * std_amt)
println("Max Amount in Data: ", maximum(df.amount))
println("Fraud count: ", sum(df.is_fraud))